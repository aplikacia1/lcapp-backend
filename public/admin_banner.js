...
async function loadList(){
  ...
  try{
    ...
    if (!items.length) { list.innerHTML = '<div class="muted">Zatiaľ žiadne bannery.</div>'; return; }

    list.innerHTML = items.map(b => `
      <div class="banner-tile" data-id="${b._id}">
        <img src="/uploads/${b.image}" alt="${escapeHtml(b.title || '')}">
        <div class="tile-body">
          <div><strong>${escapeHtml(b.title || 'Bez názvu')}</strong></div>
          <div class="muted">${b.isActive ? 'Aktívny' : 'Neaktívny'} • ${new Date(b.createdAt).toLocaleString('sk-SK')}</div>
