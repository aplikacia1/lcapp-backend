...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function findBalconySystem(heightDom, drainDom) {
    ...
    return (
      window.BALCONY_SYSTEMS.find(
        (sys) =>
          sys.heightCategory === hCfg && sys.drainType === dCfg
      ) || null
    );
  }

  // ---------------------------------------------------------------------------
  // PREVIEW obrázky – 9 kombinácií
  // ---------------------------------------------------------------------------
  ...
  const state = {
    ...
    pendingAction: null, // download | email | offer
  ...
  const goToStep3Btn =
    document.getElementById("goToStep3Btn");

  // === KONIEC ČASTI 1 (riadky ~1–500) ===
  // ---------------------------------------------------------------------------
  // ELEMENTY – KROK 3
  // ---------------------------------------------------------------------------
  ...
  const bomPerimeterEl =
    document.getElementById("bomPerimeter");
  ...
  const goToStep4Btn =
    document.getElementById("goToStep4Btn");
  const backToStep2Btn =
    document.getElementById("backToStep2Btn");
  ...
  const k4PerimeterEl =
    document.getElementById("k4Perimeter");

  const k4HeightLabelEl =
    document.getElementById("k4HeightLabel");
  const k4DrainLabelEl =
    document.getElementById("k4DrainLabel");
  const k4SystemNameEl =
    document.getElementById("k4SystemName");

  const k4BomAreaEl =
    document.getElementById("k4BomArea");
  ...
  // PDF TLAČIDLÁ
  // ---------------------------------------------------------------------------
  const btnPdfDownload =
    document.getElementById("btnPdfDownload");
  const btnPdfToEmail =
    document.getElementById("btnPdfToEmail");
  ...
  // ---------------------------------------------------------------------------
  const tileModal =
    document.getElementById("tileModal");
  const tileThicknessInput =
    document.getElementById("tileThicknessInput");
  const tileConfirmBtn =
    document.getElementById("tileConfirmBtn");
  const tileCloseBtn =
    document.getElementById("tileCloseBtn");

  // ---------------------------------------------------------------------------
  // PDF UI POLIA
  // ---------------------------------------------------------------------------
  const pdfCustomerNameEl =
    document.getElementById("pdfCustomerName");
  const pdfCustomerCompanyEl =
    document.getElementById("pdfCustomerCompany");
  const pdfCustomerEmailEl =
    document.getElementById("pdfCustomerEmail");
  const pdfShowEmailEl =
    document.getElementById("pdfShowEmail");

  const pdfAutoNameInfoEl =
    document.getElementById("pdfAutoNameInfo");
  const pdfAutoNameTextEl =
    document.getElementById("pdfAutoNameText");

  // ---------------------------------------------------------------------------
  // POMOCNÉ FUNKCIE – validácie
  // ---------------------------------------------------------------------------
  function parseVal(v) {
  ...
  function hasRecipientEmailForMailOrOffer() {
    ...
    return !!(state.pdfCustomerEmail || "").trim();
  }
  ...
  function updatePdfButtons() {
    ...
    if (btnPdfToEmail)
      btnPdfToEmail.disabled =
        !(ok && hasRecipientEmailForMailOrOffer());
    if (btnPdfRequestOffer)
      btnPdfRequestOffer.disabled =
        !(ok && hasRecipientEmailForMailOrOffer());
  }
  ...
  function updateStep3Summary() {
    if (k3TypeEl)
      k3TypeEl.textContent =
        state.fromStep1TypeLabel || "–";
    if (k3ShapeEl)
      k3ShapeEl.textContent =
        state.shapeKey || "–";
  ...
  // === KONIEC ČASTI 2 (riadky ~501–1000) ===
  // ---------------------------------------------------------------------------
  // TVARY + LABELY
  // ---------------------------------------------------------------------------
  const shapeConfigs = {
  ...
  // ---------------------------------------------------------------------------
  // “kótovanie” v náhľade (ak máš v HTML data-preview-side)
  // ---------------------------------------------------------------------------
  ...
  function updateShapePreviewLabels() {
    ...
    nodes.forEach((el) => {
      ...
      el.textContent = val != null ? formatLen(val) : side;
  ...
  function buildNormalizedWallSides() {
    ...
    if (state.shapeKey === "square") return { A: !!w.A, B: !!w.B, C: !!w.C, D: !!w.D };
    if (state.shapeKey === "rectangle") return { A: !!w.A, B: !!w.B, C: !!w.C, D: !!w.D };
    if (state.shapeKey === "l-shape") return { ...w };
    return { ...w };
  }

  // ---------------------------------------------------------------------------
  // STRANY PRI STENE – checkboxy s data-side + label.wall-box-label
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // Načítanie mena prihláseného používateľa (ak endpoint existuje)
  // ---------------------------------------------------------------------------
  async function tryLoadLoggedUserName() {
    ...
    try {
      ...
      const nick = (u?.name || "").toString().trim();
      ...
      if (pdfCustomerCompanyEl && !String(pdfCustomerCompanyEl.value || "").trim() && company) {
  ...
  // ---------------------------------------------------------------------------
  // Navrhovaná skladba + BOM (orientačne)
  // ---------------------------------------------------------------------------
  ...
  function updateBom() {
    ...
    if (!state.heightDomId || !state.drainDomId) {
      resetBom("Najprv v kroku 3 zvoľte konštrukčnú výšku a spôsob odtoku vody.");
      return;
    }

    if (!state.system) {
      resetBom("Pre túto kombináciu výšky a odtoku vody zatiaľ nemáme definovanú skladbu.");
      return;
    }
    ...
    if (area == null || per == null) {
      resetBom("Na výpočet materiálu potrebujeme mať doplnené rozmery (plocha aj obvod).");
      return;
    }

    // orientačne: profil 2.5m, lepidlo 5 m2
  ...
  // ---------------------------------------------------------------------------
  // PREPOČET – čítanie inputov + prepočty
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // DRAIN availability (High nemá edge-free)
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // LISTENERY – KROK 2 (tvar + rozmery + steny)
  // ---------------------------------------------------------------------------
  if (shapeGrid) {
  ...
  // ---------------------------------------------------------------------------
  // LISTENERY – KROK 4 (späť)
  // ---------------------------------------------------------------------------
  if (backToStep3Btn) backToStep3Btn.addEventListener("click", () => showStep(3));

  // === KONIEC ČASTI 3 (pokračujeme PDF + tile modal + OFFER v ďalšej časti) ===
  // ---------------------------------------------------------------------------
  // ✅ BARA podľa hrúbky dlažby (mm) – pre PDF
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // TILE MODAL – vyžiadanie hrúbky pred PDF akciami
  // ---------------------------------------------------------------------------
  ...
  async function runPendingActionIfAny() {
    ...
    if (a === "offer") return postPdfOffer(true);
  }
  ...
  // ---------------------------------------------------------------------------
  // ✅ PAYLOAD – jednotný pre PDF / email / ponuku
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // ENDPOINTY – download / email / offer
  // ---------------------------------------------------------------------------
  async function tryPdfDownloadEndpoints(payload) {
    ...
    for (const url of candidates) {
      try {
        ...
        if (!ct.includes("pdf") && !ct.includes("octet-stream")) {
          const txt = await res.text().catch(() => "");
          lastErr = new Error(`${url}: Nečakaný content-type (${ct || "?"})${txt ? " - " + txt : ""}`);
          continue;
        }
    ...
    return { ok: false, error: lastErr || new Error("Nepodarilo sa nájsť funkčný PDF endpoint.") };
  }
  ...
  async function tryPdfOfferEndpoints(payload) {
    const url = "/api/pdf/balkon-final-html-offer";
    try {
      const res = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  // ---------------------------------------------------------------------------
  // PDF AKCIE – download / email / offer
  // ---------------------------------------------------------------------------
  ...
  async function postPdfEmail(skipTileCheck = false) {
    ...
    try {
      ...
      alert(out.data?.message || "PDF bolo odoslané e-mailom.");
    } catch (e) {
      alert(`Chyba pri odosielaní e-mailu: ${e?.message || e}`);
    } finally {
      if (btn) {
        btn.textContent = oldText;
        updatePdfButtons();
  ...
  async function postPdfOffer(skipTileCheck = false) {
    ...
    try {
      ...
      const payload = buildBridgePayload();
      const out = await tryPdfOfferEndpoints(payload);
      if (!out.ok) throw out.error;

      alert(out.data?.message || "Požiadavka o ponuku bola odoslaná.");
    } catch (e) {
      alert(`Chyba pri odosielaní ponuky: ${e?.message || e}`);
    } finally {
      if (btn) {
        btn.textContent = oldText;
        updatePdfButtons();
  ...
  if (btnPdfRequestOffer) btnPdfRequestOffer.addEventListener("click", () => postPdfOffer(false));

  // ---------------------------------------------------------------------------
  // Preview modal (krok 4)
  // ---------------------------------------------------------------------------
  ...
  if (previewModal) {
    const bd = previewModal.querySelector(".preview-backdrop");
    if (bd) bd.addEventListener("click", closePreview);
  }
  ...
  // default tvar
  setShape(state.shapeKey || "square");
  recomputeFromInputs();

  // inicializácia UI v kroku 3/4
  updateStep3Summary();
  updateRecommendedBox();
  updateBom();
  updateStep4Summary();
