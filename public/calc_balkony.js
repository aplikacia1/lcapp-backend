...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---------------------------------------------------------------------------
  const HEIGHT_DOM_TO_CONFIG = { low: "LOW", medium: "MEDIUM", high: "HIGH" };
  ...
  function updatePdfButtons() {
    ...
    if (btnPdfToEmail)
      btnPdfToEmail.disabled = !(ok && hasRecipientEmailForMailOrOffer());
    if (btnPdfRequestOffer)
      btnPdfRequestOffer.disabled = !(ok && hasRecipientEmailForMailOrOffer());
  }
  ...
  function applyWallOptionsForCurrentShape() {
    ...
    const allowed = new Set(
      (cfg && cfg.wallOptions) || ["A", "B", "C", "D", "E", "F"]
    );
  ...
  if (pdfCustomerNameEl)
    pdfCustomerNameEl.addEventListener("input", syncPdfUiToState);
  if (pdfCustomerCompanyEl)
    pdfCustomerCompanyEl.addEventListener("input", syncPdfUiToState);
  if (pdfCustomerEmailEl)
    pdfCustomerEmailEl.addEventListener("input", syncPdfUiToState);
  if (pdfShowEmailEl)
    pdfShowEmailEl.addEventListener("change", updatePdfUiEnabled);
  ...
  function buildBridgePayload() {
    ...
    const bom = {
      area: bomAreaEl
        ? parseFloat(String(bomAreaEl.textContent).replace(",", "."))
        : null,
  ...
  async function tryPdfDownloadEndpoints(payload) {
    ...
    return {
      ok: false,
      error:
        lastErr || new Error("Nepodarilo sa nájsť funkčný PDF endpoint."),
    };
  }
  ...
  async function postPdfEmail(skipTileCheck = false) {
    ...
    if (!hasRecipientEmailForMailOrOffer()) {
      alert(
        "Chýba e-mail pre odoslanie. Prihláste sa, alebo zadajte e-mail v údajoch do PDF."
      );
      return;
    }
  ...
  if (btnPdfDownload)
    btnPdfDownload.addEventListener("click", () => postPdfDownload(false));
  if (btnPdfToEmail)
    btnPdfToEmail.addEventListener("click", () => postPdfEmail(false));
  ...
  if (btnPdfRequestOffer) {
    btnPdfRequestOffer.addEventListener("click", () => {
      ...
      if (!hasRecipientEmailForMailOrOffer()) {
        alert(
          "Chýba e-mail pre ponuku. Prihláste sa, alebo zadajte e-mail v údajoch do PDF."
        );
        return;
      }
      ...
      alert(
        "Ponuka: zatiaľ len pripravujeme. Neskôr dopojíme odoslanie požiadavky technikovi."
      );
    });
  }
